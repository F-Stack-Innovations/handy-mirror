...
const EditInformationScreen = () => {
  ...
  useEffect(() => {
    const loadPhotoFromAsyncStorage = async () => {
      try {
        const savedPhotoAsync = await AsyncStorage.getItem('photoAsync');
        if (savedPhotoAsync) {
  ...
  const takePhoto = async () => {
    ...
    if (!photoResult.canceled) {
      ...
      try {
        await AsyncStorage.setItem('photoAsync', photoResult.assets[0].uri);
      } catch (error) {
        console.log(
        ...
        <View style={styles.titleContainer}>
          ...
          <Pressable style={styles.goBack} onPress={takePhoto}>
            <View style={styles.photoCircleContainer}>
              {photo ? (
                <Image style={styles.photoCircleText} source={{ uri: photo }} />
              ) : (
        ...
        <View style={styles.formulaireContainer}>
          ...
          <View style={styles.inputTextContainer}>
            <Text style={styles.inputTextPlaceholder}>
              Email - Attention il s'agit de l'identifiant de connexion
            </Text>
            <TextInput
              style={styles.inputTextSaisie}
