...
const getUserByID = async (req, res) => {
  try {
    ...
    if (!checkValidUserID) {
      ...
      const getUserID = await UserModel.findOne({ _id: userID }).populate(
        'modules',
      );
    ...
    if (!checkValidUserID) {
      ...
      const updatedUser = await UserModel.findByIdAndUpdate(
        userID,
        {
          ...
          compagnionLife,
          children,
          photoProfile: photoProfile ? photoProfile : '',
        },
        {
          new: true,
        },
  ...
  try {
    ...
    if (!checkValidUserID) {
      ...
      if (!checkUserExist) {
        return res.status(404).json({
          error:
            "Suppression impossible, il n'existe aucun utilisateur avec cet identifiant",
        });
      } else {
